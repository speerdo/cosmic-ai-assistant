...
| **Phase 2** | In progress (2026-09-17), spec `docs/phase2-plan.md`. §2.2 core types done and reviewed (71 tests green; one recorded deviation — async `synthesize`, findings §2). **The review found five defects, all on ticked boxes** — three in the WAV codec, including 24-bit files decoding as near-silence and a zero sample rate panicking inside the encoder (findings **§R**). §2.0 audited: `clang`/`cmake`/`libpipewire-0.3-dev` missing, install needs a manual sudo run; **§2.1 *and* §2.3 are blocked on it**, and with them §2.4's "say speaks" DoD. |
